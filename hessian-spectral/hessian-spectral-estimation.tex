...
\newcommand{\lab}{\href{https://samyakjainberk.github.io/hessian-spectral/}{interactive widget}} % update URL
...
\]
so in exact arithmetic each new basis vector needs orthogonalization against only the \emph{two} previous ones, not all of them. This is easier to implement as it overcomes the floating point errors which would occur if the Gram-Schmidt was done at the end. Thus the lanczos iteration, which one would find in most places is the following:
...
B_j \ \text{from the QR step } W_j = Q_{j+1} B_j,
\]
...
\]
Note that the Lanczos run hands us exactly these quantities: the Ritz values $\theta_j$ stand in for the eigenvalues $\lambda_i$, and $(S_{1j})^2$ stands in for the weight $(u_j^\top v_1)^2$.
...
\qquad
\hat\mu_k = \frac{1}{p} \Tr\, T_k(\tilde A) .
\]
\end{resultbox}
...
Lanczos & extreme eigenpairs & $O(knp) + O(k^2p)$ reorth. \\
...
Three tabs use ideas we have not defined yet:
\begin{itemize}
  \item \textbf{Edge + deflated SLQ} (tab 6). First run a few rounds of Lanczos and collect the converged extreme Ritz vectors into an orthonormal $U$. Then run SLQ on the deflated operator $A' = (I - UU^\top)\, A\, (I - UU^\top)$, with probes kept orthogonal to $U$. The harvested eigenvalues enter the density exactly, and SLQ fills in the rest:
  \[
